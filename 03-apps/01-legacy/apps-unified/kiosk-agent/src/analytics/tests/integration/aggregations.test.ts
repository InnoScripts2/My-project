...
describe('Aggregations Integration', () => {
  ...
  const tempRoot = path.join(os.tmpdir(), 'kiosk-agent-tests', 'aggregations');

  before(async () => {
    fs.mkdirSync(tempRoot, { recursive: true });
    sqliteDbPath = path.join(tempRoot, 'source.db');
    aggregationsDir = path.join(tempRoot, 'aggregations');
    fs.mkdirSync(aggregationsDir, { recursive: true });

    if (fs.existsSync(sqliteDbPath)) fs.unlinkSync(sqliteDbPath);
    const existing = fs.readdirSync(aggregationsDir);
  ...
  it('should run daily aggregation and create output files', async () => {
    ...
    const testDbPath = path.join(tempRoot, 'data', 'sessions.db');
