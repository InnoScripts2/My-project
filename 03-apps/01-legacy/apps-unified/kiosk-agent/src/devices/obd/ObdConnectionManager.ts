...
import { KingbolenEdiagDriver, type KingbolenEdiagMetrics, type KingbolenEdiagOptions } from '@selfservice/obd-diagnostics';
...
type ObdDriverFactory = (options: KingbolenEdiagOptions & { autoReconnect?: boolean }) => KingbolenEdiagDriver;

const defaultObdDriverFactory: ObdDriverFactory = (options) => new KingbolenEdiagDriver(options);
}
...
export class ObdConnectionManager {
  ...
  private readonly driverFactory: ObdDriverFactory;
  ...
  constructor(options: ObdConnectionManagerOptions = {}, driverFactory: ObdDriverFactory = defaultObdDriverFactory) {
