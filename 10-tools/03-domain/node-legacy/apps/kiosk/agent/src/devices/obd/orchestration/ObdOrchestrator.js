...
import { SessionStatus, InMemorySessionStore, } from './Session.js';
import { ObdSessionError, ObdStateError } from './errors.js';
import { readFileSync } from 'fs';
export var OrchestratorState;
(function (OrchestratorState) {
    OrchestratorState["DISCONNECTED"] = "DISCONNECTED";
    OrchestratorState["CONNECTING"] = "CONNECTING";
    OrchestratorState["CONNECTED"] = "CONNECTED";
    OrchestratorState["SCANNING"] = "SCANNING";
    OrchestratorState["RESULTS_READY"] = "RESULTS_READY";
    OrchestratorState["IDLE"] = "IDLE";
    OrchestratorState["ERROR"] = "ERROR";
})(OrchestratorState || (OrchestratorState = {}));
export class ObdOrchestrator extends EventEmitter {
    driver;
    state = OrchestratorState.DISCONNECTED;
    currentSessionId;
    sessionStore;
    config;
    scanTimeoutTimer;
    ...
    async startScan(metadata) {
        if (this.state !== OrchestratorState.CONNECTED && this.state !== OrchestratorState.IDLE) {
            throw new ObdStateError('Cannot start scan in current state', 'invalid_state', this.state, 'startScan');
        }
        if (this.activeSessions.size >= this.config.maxConcurrentSessions) {
            throw new ObdSessionError('Maximum concurrent sessions reached', 'max_sessions_reached', undefined, { maxSessions: this.config.maxConcurrentSessions });
        }
    ...
    async runScanSequence(sessionId) {
        try {
            this.progress = 10;
    ...
    handleScanError(sessionId, error) {
        const session = this.sessionStore.get(sessionId);
        if (session) {
    ...
    async clearDtc(confirm) {
        if (confirm !== true) {
            throw new ObdSessionError('Confirmation required to clear DTC', 'confirmation_required');
        }
        if (this.state !== OrchestratorState.RESULTS_READY && this.state !== OrchestratorState.IDLE) {
            throw new ObdStateError('Cannot clear DTC in current state', 'invalid_state', this.state, 'clearDtc');
        }
        try {
