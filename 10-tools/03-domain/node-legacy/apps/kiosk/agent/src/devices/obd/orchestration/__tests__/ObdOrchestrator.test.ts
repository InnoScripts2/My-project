...
import type { DeviceObd, DtcEntry, PidValue, ObdConfig, ObdStatus } from '../../driver/DeviceObd.js';

class MockDriver extends EventEmitter implements DeviceObd {
  private status: ObdStatus = 'disconnected' as ObdStatus;
  ...
  async init(config: ObdConfig): Promise<void> {
    ...
    this.status = 'ready' as ObdStatus;
  ...
  async disconnect(): Promise<void> {
    this.status = 'disconnected' as ObdStatus;
...
describe('ObdOrchestrator', () => {
  describe('connect', () => {
    ...
    it('should emit error on connection failure', async () => {
      ...
      await assert.rejects(
        async () => orchestrator.connect(),
        (err: any) => err instanceof ObdSessionError && err.code === 'connection_failed'
      );
    });
  ...
  describe('startScan', () => {
    ...
    it('should throw error if not connected', async () => {
      ...
      await assert.rejects(
        async () => orchestrator.startScan(),
        (err: any) => err instanceof ObdStateError && err.code === 'invalid_state'
      );
    });
    ...
    it('should complete scan and transition to RESULTS_READY', async (t) => {
      ...
      return new Promise((resolve, reject) => {
        orchestrator.on('scan-complete', (sessionId: string) => {
          try {
            const status = orchestrator.getStatus();
    ...
    it('should track scan progress', async (t) => {
      ...
      return new Promise((resolve, reject) => {
        ...
        orchestrator.on('scan-progress', (progress: number) => {
  ...
  describe('clearDtc', () => {
    it('should require confirmation', async () => {
      ...
      await assert.rejects(
        async () => orchestrator.clearDtc(false),
        (err: any) => err instanceof ObdSessionError && err.code === 'confirmation_required'
      );
    });
    ...
    it('should clear DTC codes when confirmed', async (t) => {
      ...
      return new Promise((resolve, reject) => {
        orchestrator.on('scan-complete', async (sessionId: string) => {
          try {
