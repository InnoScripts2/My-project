...
import {
  ...
  type UdsDataIdentifier,
  type UdsReadDataResponse,
} from './UdsTypes.js';

export class UdsProtocol {
  constructor(private driver: ObdDriver) {}

  /**
   * Switch diagnostic session
   */
  async switchSession(session: UdsSessionType): Promise<void> {
   ...
   */
  async readDataByIdentifier(did: UdsDataIdentifier): Promise<UdsReadDataResponse> {
   ...
   */
  async readMultipleDataByIdentifier(dids: UdsDataIdentifier[]): Promise<UdsReadDataResponse[]> {
    const results: UdsReadDataResponse[] = [];
   ...
   */
  async ecuReset(resetType: number = 0x01, confirmed: boolean = false): Promise<void> {
    if (!confirmed) {
   ...
   */
  private parseDidData(did: UdsDataIdentifier, data: Buffer): any {
    switch (did) {
      case 0xF190: // VIN
