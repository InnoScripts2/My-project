...
export class ScheduledAggregations {
    constructor(analyticsService, exportService, aggregationsDir) {
     ...
     */
    scheduleDailyAggregation(cronExpression = '0 2 * * *') {
        if (this.dailyTask) {
            this.dailyTask.stop();
        }
        ...
        console.log(`[ScheduledAggregations] Daily aggregation scheduled with cron: ${cronExpression}`);
    }
    /**
     * Запланировать еженедельный отчёт
     */
    scheduleWeeklyReport(cronExpression = '0 3 * * 1') {
        if (this.weeklyTask) {
        ...
        console.log(`[ScheduledAggregations] Weekly report scheduled with cron: ${cronExpression}`);
    }
    /**
    ...
    async runDailyAggregation() {
        ...
        const aggregationPath = path.join(this.aggregationsDir, `aggregation-${dateStr}.json`);
    ...
    async runWeeklyReport() {
        ...
        const reportPath = path.join(this.aggregationsDir, `weekly-report-${startDate.toISOString().split('T')[0]}.json`);
