...
export class AnalyticsService {
    ...
    async loadSingleSource(source) {
        ...
        switch (source.type) {
            ...
            case 'csv':
                // Load from CSV
                await runAsync(`COPY ${source.tableName} FROM '${source.path}' (HEADER, DELIMITER ',')`);
                break;
            case 'json':
                // Load from JSON
                await runAsync(`INSERT INTO ${source.tableName} SELECT * FROM read_json_auto('${source.path}')`);
                break;
            case 'parquet':
                // Load from Parquet
                await runAsync(`INSERT INTO ${source.tableName} SELECT * FROM read_parquet('${source.path}')`);
                break;
            default:
     ...
     */
    async getSessionsReport(filter) {
    ...
    async getErrorsReport(filter) {
      ...
      `;
            ...
            timer();
            return {
                topErrors: result.rows.map((row) => ({
                    code: row.code,
                    description: row.description,
                    count: row.count,
                    severity: row.severity,
                })),
                totalErrors: result.rows.reduce((sum, row) => sum + row.count, 0),
                byDevice,
            };
        }
        catch (error) {
