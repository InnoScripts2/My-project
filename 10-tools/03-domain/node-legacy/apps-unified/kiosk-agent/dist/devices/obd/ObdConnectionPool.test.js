...
describe('ObdConnectionPool', () => {
    ...
    it('should timeout queued requests', async () => {
        ...
        await pool.acquireConnection('vehicle1', 5000);
        // This should timeout
        await assert.rejects(pool.acquireConnection('vehicle2', 100), { message: 'Connection acquisition timeout' });
        await pool.shutdown();
    });
    it('should prevent duplicate connections for same vehicle', async () => {
        const pool = new ObdConnectionPool(3);
        await pool.acquireConnection('vehicle1', 5000);
        await assert.rejects(pool.acquireConnection('vehicle1', 5000), { message: 'Vehicle already has active connection' });
        await pool.shutdown();
    });
