...
import { ObdConnectionError, ObdTimeoutError, ObdParseError, ObdUnsupportedError, } from './errors.js';
...
export class Elm327Driver extends EventEmitter {
    ...
    async init(config) {
        ...
        try {
            ...
            this.updateStatus(ObdStatus.ERROR);
            throw new ObdConnectionError(`Failed to initialize OBD adapter: ${error instanceof Error ? error.message : String(error)}`, { config: this.config });
        }
    }
    /**
    ...
    async readDtc() {
        ...
        try {
            ...
            throw new ObdConnectionError(`Failed to read DTC: ${error instanceof Error ? error.message : String(error)}`);
        }
        finally {
    ...
    async clearDtc() {
        ...
        try {
            ...
            throw new ObdConnectionError(`Failed to clear DTC: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    /**
    ...
    async readPid(pid) {
        ...
        try {
            ...
            throw new ObdConnectionError(`Failed to read PID ${pidUpper}: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    /**
     * Get current status
    ...
    async initializeAdapter() {
        ...
        for (const { cmd, desc } of initSequence) {
            try {
                ...
                throw new ObdConnectionError(`Initialization failed at ${desc}`, { command: cmd, error: error instanceof Error ? error.message : String(error) });
            }
        }
     ...
     */
    sendCommand(command, priority, timeout, retries = this.config.retries) {
        ...
        try {
            ...
            const details = error instanceof Error
                ...
                : typeof error === 'object' && error !== null
                    ? error
                    : { value: String(error) };
            throw new ObdParseError('Failed to parse DTC response', response, details);
        }
    }
    /**
     * Parse PID response
        ...
        try {
            ...
            const details = error instanceof Error
                ...
                : typeof error === 'object' && error !== null
                    ? error
                    : { value: String(error) };
            throw new ObdParseError(`Failed to parse PID ${pid} response`, response, details);
        }
    }
    /**
