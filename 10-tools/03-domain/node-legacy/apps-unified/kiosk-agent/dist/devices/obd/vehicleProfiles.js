 ...
 */
export function getVehicleProfile(make, year) {
    ...
    const candidates = ALL_PROFILES.filter(profile => {
        ...
        if (year && profile.yearRange) {
            if (profile.yearRange.min && year < profile.yearRange.min)
                return false;
            if (profile.yearRange.max && year > profile.yearRange.max)
                return false;
        }
        return true;
    });
    ...
    if (year) {
        const genericModern = candidates.find(p => p.make === 'Generic' && p.model === 'Modern (2008+)');
        const genericLegacy = candidates.find(p => p.make === 'Generic' && p.model === 'Legacy (pre-2008)');
        if (year >= 2008 && genericModern)
            return genericModern;
        if (year < 2008 && genericLegacy)
 ...
 */
export function getProtocolSequence(make, year) {
 ...
 */
export function getRecommendedTimeout(make, year) {
    const profile = getVehicleProfile(make, year);
 ...
 */
export function requiresSlowInit(make, year) {
    const profile = getVehicleProfile(make, year);
 ...
 */
export function getInitCommands(make, year) {
    const profile = getVehicleProfile(make, year);
