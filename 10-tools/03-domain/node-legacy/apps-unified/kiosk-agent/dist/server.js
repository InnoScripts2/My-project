...
import { KioskAgent } from './main.js';
import { writeReportToOutbox } from './reports/service.js';
import { getMailConfigFromEnv, sendReportEmail } from './reports/mailer.js';
import * as path from 'path';
import { getDevicesStatus } from './devices.js';
export function createServer(env = 'DEV') {
    const agent = new KioskAgent({ env, logLevel: 'info' });
    ...
    const server = http.createServer((req, res) => {
        ...
        if (url.pathname === '/reports/generate' && req.method === 'POST') {
            if (env === 'PROD')
                return forbidden('reports_disabled_in_prod');
            return readJson(async (body) => {
                try {
                    const data = body?.data;
                    if (!data || !data.sessionId || !data.contact)
                        return badRequest('data_required');
        ...
        if (url.pathname === '/reports/send' && req.method === 'POST') {
            if (env === 'PROD')
                return forbidden('reports_send_disabled_in_prod_until_psp');
            const mailCfg = getMailConfigFromEnv();
            if (!mailCfg)
                return notImplemented('email_not_configured');
            return readJson(async (body) => {
                try {
                    const data = body?.data;
                    if (!data || !data.sessionId || !data.contact)
