...
<body>
  ...
  `
    ...
    async checkKioskMode() {
        try {
            if (this.platform === 'Windows') {
                const result = execSync('powershell -Command "Get-AssignedAccess | ConvertTo-Json"', { encoding: 'utf-8', timeout: 5000 }).trim();
    ...
    async checkSystemShortcutsBlocked() {
        return {
            ...
            status: 'warning',
            details: 'Manual verification required',
            remediation: 'Block Alt+Tab, Win, Ctrl+Alt+Del via Group Policy or compositor rules',
        };
    }
    async checkUpdatesScheduled() {
        return {
    ...
    async checkFirewallRules() {
        try {
            if (this.platform === 'Windows') {
                const rules = execSync('netsh advfirewall firewall show rule name=all dir=out', { encoding: 'utf-8', timeout: 5000 });
                return {
    ...
    async checkHomeDirectoryRestricted() {
        return {
    ...
    async checkUnnecessaryServicesDisabled() {
        return {
