 ...
 */
import { createSecurityRoutes, createSecurityMetrics, AuditLogger, HardeningChecklist, } from './index.js';
export async function initializeSecurity(app, registry) {
    console.log('Initializing security modules...');
    const auditLogger = new AuditLogger();
    const securityMetrics = createSecurityMetrics(registry);
    await auditLogger.logEvent('SystemEvent', 'agent_started', 'system', {
    ...
    if (process.env.AGENT_ENV === 'PROD') {
        ...
        if (report.overallStatus === 'failed') {
            ...
            console.error('Failed checks:', report.checks.filter((c) => c.status === 'failed'));
            console.error('Recommendations:', report.recommendations);
            await auditLogger.logEvent('SystemEvent', 'hardening_check_failed', 'system', {
    ...
    const cleanupInterval = setInterval(async () => {
        try {
            ...
            console.error('Failed to cleanup audit logs:', error instanceof Error ? error.message : String(error));
        }
    }, 24 * 60 * 60 * 1000);
    process.on('SIGTERM', () => {
