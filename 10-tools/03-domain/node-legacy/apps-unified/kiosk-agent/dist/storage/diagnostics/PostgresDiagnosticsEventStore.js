...
function getPgPool() {
    ...
    const { PGHOST = '127.0.0.1', PGPORT = '5432', PGUSER = 'postgres', PGPASSWORD = 'postgres', PGDATABASE = 'kiosk', } = process.env;
    return new Pool({
        host: PGHOST,
        port: Number(PGPORT),
        user: PGUSER,
 ...
 */
export function createPostgresDiagnosticsEventStore() {
  ...
  `;
    ...
    function buildConnectionSummaryFromEvents(events, captureMs, since) {
        ...
        let currentSinceMs = parseTs(since) ?? parseTs(events[0]?.at) ?? captureMs;
        ...
        for (const ev of events) {
            ...
            if (ev.previousState && ev.previousState !== ev.state)
                transitions += 1;
    ...
    function computeMeanTimeBetweenFailures(events) {
        const failures = events.filter((e) => e.type === 'operation' && e.outcome === 'failure').map((e) => Date.parse(e.at)).filter(Number.isFinite);
        if (failures.length < 2)
            return null;
        let total = 0;
        for (let i = 1; i < failures.length; i++)
            total += (failures[i] - failures[i - 1]);
        return Math.round(total / (failures.length - 1));
    }
    function computeMeanTimeToRecovery(events) {
        let total = 0, count = 0;
        for (let i = 0; i < events.length; i++) {
            const ev = events[i];
            if (!(ev.type === 'operation' && ev.outcome === 'failure'))
                continue;
            const failMs = Date.parse(ev.at);
            if (!Number.isFinite(failMs))
                continue;
            for (let k = i + 1; k < events.length; k++) {
                const cand = events[k];
                if (cand.type !== 'operation')
                    continue;
                const recMs = Date.parse(cand.at);
                if (!Number.isFinite(recMs) || recMs <= failMs)
                    continue;
    ...
    function computeCurrentFailureStreak(events) {
        let streak = 0;
    ...
    function summarizeFromEvents(events, options = {}) {
            ...
            if (dateKey) {
                ...
                if (!bucket) {
                    bucket = {
                        ...
                        success: 0,
                        failure: 0,
                        byOp: OPERATIONS.reduce((acc, op) => { acc[op] = { success: 0, failure: 0 }; return acc; }, {}),
                    };
        ...
        for (const v of Object.values(totalsByState))
            totalConnectionDuration += v;
    ...
    const store = {
        ...
        prune(beforeIso) {
            pool.query(pruneText, [beforeIso]).catch(() => { });
        },
        summarize(options = {}) {
            try {
                ...
                return {
                    ...
                    rolling: {
                        last7Days: { windowDays: 7, from: new Date(now - 7 * 86400000).toISOString(), until: new Date(now).toISOString(), total: 0, success: 0, failure: 0, successRate: 0, failureRate: 0, byOperation: OPERATIONS.reduce((acc, op) => { acc[op] = { total: 0, success: 0, failure: 0, successRate: 0, failureRate: 0 }; return acc; }, {}), topFailures: [] },
                        last30Days: { windowDays: 30, from: new Date(now - 30 * 86400000).toISOString(), until: new Date(now).toISOString(), total: 0, success: 0, failure: 0, successRate: 0, failureRate: 0, byOperation: OPERATIONS.reduce((acc, op) => { acc[op] = { total: 0, success: 0, failure: 0, successRate: 0, failureRate: 0 }; return acc; }, {}), topFailures: [] },
                    },
                };
            }
        },
