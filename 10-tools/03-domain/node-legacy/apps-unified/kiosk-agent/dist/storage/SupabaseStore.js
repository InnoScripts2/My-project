...
function createSupabase() {
    ...
    if (!supabaseUrl || !supabaseKey) {
        throw new Error('SUPABASE_URL и SUPABASE_SERVICE_ROLE_KEY должны быть установлены для SupabaseStore. ' +
            'Проверьте .env файл.');
    }
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        try {
            ...
            const result = await Promise.race([
                fn(),
                new Promise((_, reject) => setTimeout(() => reject(new Error('Operation timeout')), RETRY_CONFIG.timeout))
            ]);
