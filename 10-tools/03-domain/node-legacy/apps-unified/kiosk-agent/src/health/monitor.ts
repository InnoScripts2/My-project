...
import { getSystemInfo, type SystemInfo } from './healthCheck.js';
...
export class HealthMonitor {
  ...
  private eventLoopLagP95 = 0;
  private eventLoopSamples: number[] = [];

  constructor(
    private readonly checkIntervalSeconds: number = 60
  ) {
   ...
   * Get health history
   */
  getHistory(limit = 10): HealthStatus[] {
  ...
  async checkReady(
    ...
    try {
      ...
      if ('healthCheck' in store && typeof store.healthCheck === 'function') {
        const result = await (store as any).healthCheck();
        checks['sqlite'] = {
  ...
  async checkDeep(
    ...
    const syntheticChecks: Record<string, ComponentHealth> = {};
    ...
    try {
      ...
      if ('withTransaction' in store && typeof store.withTransaction === 'function') {
        await (store as any).withTransaction(async (db: any) => {
   ...
   */
  private async testStoreConnection(store: PersistenceStore): Promise<void> {
    if (store.ping) {
      await store.ping();
    } else {
      // Fallback test
