...
async function main() {
  ...
  const tempRoot = path.join(os.tmpdir(), 'kiosk-agent-analytics-example');
  const dbPath = path.join(tempRoot, 'example-analytics.duckdb');
  const exportDir = path.join(tempRoot, 'exports');
  const aggregationsDir = path.join(tempRoot, 'aggregations');
