...
import {
  ...
  type DiagnosticOperationEvent,
  type DiagnosticTimelineEvent,
} from './DiagnosticSessionManager.js';
import type { DiagnosticsEventStoreSummary } from '../../storage/sqlite/diagnosticsEventStore.js';
import type { ObdConnectionSnapshot } from './ObdConnectionManager.js';
import { obdConnectionManager } from './ObdConnectionManager.js';

type SnapshotListener = (snapshot: ObdConnectionSnapshot) => void;

class FakeConnectionManager {
  private listener?: SnapshotListener;
  snapshot: ObdConnectionSnapshot = { state: 'disconnected', reconnectAttempts: 0 };

  addSnapshotListener(listener: SnapshotListener): () => void {
  ...
  update(partial: Partial<ObdConnectionSnapshot>): void {
...
test('DiagnosticSessionManager follows connection snapshots', () => {
  ...
  const manager = new DiagnosticSessionManager(fake as any);

  let snapshot: DiagnosticStateSnapshot = manager.getSnapshot();
...
test('DiagnosticSessionManager tracks operations and failures', async () => {
  ...
  await assert.rejects(
    () => manager.runOperation('live_data', async () => {
  ...
  const failureResult = await manager.runOperation('clear_dtc', async () => {
    return { ok: false, error: 'clear_failed' } as const;
  }, { attempts: 1, baseDelayMs: 0 });
  ...
  const operationEvents = snapshot.timeline.filter((event): event is DiagnosticOperationEvent => event.type === 'operation');
...
test('DiagnosticSessionManager timeline queries and metrics snapshot', async () => {
  ...
  await manager.runOperation('read_dtc', async () => ({ ok: true, data: [] } as const), { attempts: 1, baseDelayMs: 0 });
  await assert.rejects(
    () => manager.runOperation('live_data', async () => {
...
test('DiagnosticSessionManager provides aggregated insights', async () => {
  ...
  await manager.runOperation('status', async () => ({ ok: true, data: {} } as const), {
    attempts: 1,
    baseDelayMs: 0,
  });
...
test('DiagnosticSessionManager getHistoricalSummary delegates to persistent store', () => {
  const fake = new FakeConnectionManager();
  const expected: DiagnosticsEventStoreSummary = {
  ...
  const store = {
    ...
    record() {},
    summarize(options: unknown) {
      capturedOptions = options;
      return expected;
    },
  };

  const manager = new DiagnosticSessionManager(fake as any, store as any);
...
test('DiagnosticSessionManager forwards timeline events to external store', async () => {
  const fake = new FakeConnectionManager();
  const captured: DiagnosticTimelineEvent[] = [];
  const store = {
    enabled: true,
    driver: 'noop' as const,
