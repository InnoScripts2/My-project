function ensureLen(data, n) {
    if (data.length < n)
        throw new Error(`expected ${n} bytes, got ${data.length}`);
}
export const PID_0x0C_RPM = {
    pid: 0x0c,
    mode: 0x01,
    bytes: 2,
    name: "Engine RPM",
...
export function decodePid(pid, payload) {
    ...
    if (!def)
        throw new Error(`PID 0x${pid.toString(16)} not supported`);
