import type { BleConfig, MeasurePoint, MeasurePointId } from './types.js';
...
export interface ThicknessSessionPoint extends MeasurePoint {
  status: 'pending' | 'measured' | 'skipped';
  valueMicrons?: number;
  ts?: string;
}

export interface ThicknessSessionSnapshot {
  active: boolean;
  vehicleType?: ThkVehicleType;
  startedAt?: string;
  points: ThicknessSessionPoint[];
...
class ThicknessManagerImpl {
  private snapshot: ThicknessSnapshot = { state: 'disconnected' };
  private session: ThicknessSessionSnapshot = { active: false, points: [], measuredCount: 0, pendingCount: 0, skippedCount: 0 };

  getSnapshot(): ThicknessSnapshot {
    return { ...this.snapshot };
  }

  getSessionSnapshot(): ThicknessSessionSnapshot {
    return { ...this.session, points: this.session.points.map(p => ({ ...p })) };
  }

  async open(options: { vehicleType: ThkVehicleType; ble?: BleConfig }): Promise<{ ok: true } | { ok: false; error: string }>{
  ...
  async startSession(options: { vehicleType: ThkVehicleType }): Promise<ThicknessSessionSnapshot> {
    if (this.snapshot.state !== 'connected') {
      const err: any = new Error('thickness_not_connected');
    ...
    this.session = {
      ...
      points: basePoints.map(p => ({ ...p, status: 'pending' as const })),
      measuredCount: 0,
  ...
  markPoint(id?: MeasurePointId): { ok: true; session: ThicknessSessionSnapshot } | { ok: false; error: string }{
    if (!this.session.active) return { ok: false, error: 'thk_no_active_session' };
    const target = id
      ? this.session.points.find(p => p.id === id)
      : this.session.points.find(p => p.status === 'pending');
    if (!target) return { ok: false, error: 'thk_no_pending_points' };
...
export function getPointsTemplate(type: ThkVehicleType): MeasurePoint[] {
  const mk = (id: string, label: string): MeasurePoint => ({ id, label });
  // 40–60 точек: упрощённые шаблоны; можно расширить по мере необходимости
  const sedan: MeasurePoint[] = [
