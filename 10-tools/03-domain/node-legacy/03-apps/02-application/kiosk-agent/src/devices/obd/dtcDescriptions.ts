 ...
 */
export function parseDtcPrefix(code: string): DtcPrefix {
 ...
 */
export function describeDtc(code: string): DtcInfo {
  const normalized = normalizeDtcCode(code);
 ...
 */
function getCategoryByPrefix(prefix: DtcPrefix): string {
  switch (prefix) {
 ...
 */
export function severityFor(code: string): DtcSeverity {
  const normalized = normalizeDtcCode(code);
  
  // Misfires (P03xx) - критичны для ресурса двигателя и катализатора
  if (/^P03\d\d$/.test(normalized)) return 'critical';
  
  // Computer/ECM errors (P06xx) - могут быть критичны
  if (/^P06(0[0-9]|1[0-9])$/.test(normalized)) return 'critical';
  
  // Catalyst efficiency и mixture (P0420, P0430, P0171-P0175)
  if (normalized === 'P0420' || normalized === 'P0430') return 'warning';
  if (/^P01(71|72|74|75)$/.test(normalized)) return 'warning';
  
  // O2 sensor issues - warning
  if (/^P01[3-6]\d$/.test(normalized)) return 'warning';
  
  // Injector issues (P020x) - warning
  if (/^P020[0-8]$/.test(normalized)) return 'warning';
  
  // EGR issues (P040x) - обычно info, но могут влиять на выбросы
  if (/^P040[0-6]$/.test(normalized)) return 'info';
  
  // EVAP issues (P044x, P045x) - info
  if (/^P04(4[0-6]|5[0-6])$/.test(normalized)) return 'info';
  
  // Network/communication errors (U0xxx) - могут быть critical
  if (/^U0(100|101|102|121)$/.test(normalized)) return 'critical';
  
  // Transmission errors (P07xx) - warning
  if (/^P07\d\d$/.test(normalized)) return 'warning';
 ...
 */
export function deduplicateDtcCodes(codes: string[]): string[] {
