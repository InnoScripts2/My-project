...
// BLE библиотека для Node.js 22+
let noble: any;
try {
  const nobleModule = await import('@abandonware/noble');
  noble = nobleModule.default;
} catch (e) {
  console.warn('[KingbolenEdiag] @abandonware/noble недоступен:', e);
}
...
export class KingbolenEdiagDriver extends EventEmitter {
  ...
  async connect(): Promise<boolean> {
    ...
    if (!noble) {
      throw new Error('@abandonware/noble не доступен - установите: npm install @abandonware/noble');
    }
    ...
    return new Promise((resolve, reject) => {
      ...
      this.nobleStateChangeHandler = (state: string) => {
        if (state === 'poweredOn') {
          try { noble.startScanning([], false); } catch (e) { void e; }
        }
      };
      ...
      this.nobleDiscoverHandler = async (peripheral: any) => {
        ...
        const matchName = name?.toUpperCase?.().includes('EDIAG') || name?.toUpperCase?.().includes(this.options.deviceName.toUpperCase());
    ...
    return new Promise((resolve, reject) => {
      (async () => {
        try {
          ...
          this.txCharacteristic = characteristics.find((c: any) => c.uuid === this.options.txCharacteristicUUID);
          this.rxCharacteristic = characteristics.find((c: any) => c.uuid === this.options.rxCharacteristicUUID);
