...
async function testBleConnect(): Promise<SmokeTestResult> {
...
async function testSelfCheck(driver: KingbolenEdiagDriver): Promise<SmokeTestResult> {
...
async function testReadDtc(driver: KingbolenEdiagDriver): Promise<SmokeTestResult> {
  ...
  log('INFO', `${name}: SUCCESS - Found ${codes.length} DTC codes`);
    ...
    if (codes.length > 0) {
      codes.forEach((dtc: { code: string; description?: string }) => {
...
async function testManualConnection(): Promise<SmokeTestResult> {
