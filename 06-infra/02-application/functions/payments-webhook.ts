// Локальная версия Edge Function для теста через Deno
...
Deno.serve(async (req: Request) => {
  try {
    ...
    const rpcRes = await supabase.rpc('rpc_update_payment_status', { p_intent_id: intent_id, p_status: status, p_payload: payload })
    ...
    if (insertRes.error) console.warn('Insert webhook_events failed', insertRes.error.message)
