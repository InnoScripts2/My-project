 ...
 */
const ERROR_MAPPINGS: Record<string, { code: ObdErrorCode; subtype: ObdErrorSubtype; userMessage: string }> = {
  ENOENT: {
 ...
 */
export function normalizeObdError(
  error: unknown,
  context?: Record<string, unknown>
): NormalizedObdError {
 ...
 */
export function formatObdError(error: NormalizedObdError | unknown): string {
  if (isNormalizedObdError(error)) {
...
export function serializeObdError(error: unknown): ObdErrorResponse {
  const normalized = normalizeObdError(error);
  return {
    error: true,
 ...
 */
function isNormalizedObdError(value: unknown): value is NormalizedObdError {
  return (
    typeof value === 'object' &&
    value !== null &&
    'code' in value &&
