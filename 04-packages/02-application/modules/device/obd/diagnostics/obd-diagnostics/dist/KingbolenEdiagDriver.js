...
async function loadNoble() {
    ...
    try {
        ...
        try {
            ...
            if (req) {
                ...
                if (process.env.OBD_BLE_DEBUG)
                    console.log('[KingbolenEdiag] Используется noble-winrt (createRequire)');
                return noble;
            }
        }
        catch (eCreateReq) {
        ...
        const winrtModule = await import('noble-winrt');
        noble = winrtModule?.default ?? winrtModule;
        if (process.env.OBD_BLE_DEBUG)
            console.log('[KingbolenEdiag] Используется noble-winrt (dynamic import)');
        return noble;
    }
    catch (e1) {
        try {
...
export class KingbolenEdiagDriver extends EventEmitter {
    constructor(options = {}) {
        ...
        this.latencySuccessAccumMs = 0;
        this.latencyErrorAccumMs = 0;
        this.isActiveReconnectAttempt = false;
        this.lastReconnectStartTs = null;
        this.watchdogTimer = null;
        this.watchdogIntervalMs = 15000;
        this.options = {
    ...
    async connect(abortSignal, isReconnect = false) {
        if (this.connected)
            return true;
        ...
        return new Promise((resolve, reject) => {
            ...
            let widenTimer;
            const startWidenTimer = (initiallyFiltered) => {
                if (!initiallyFiltered || widenMs <= 0)
                    return;
                widenTimer = setTimeout(() => {
                    if (found)
                        return;
                    if (process.env.OBD_BLE_DEBUG)
                        console.log('[KingbolenEdiag] widen scan: повторный старт без сервис-фильтра');
                    try {
                        noble.stopScanning();
                    }
                    catch (e) {
                    ...
                    try {
                        noble.startScanning([], process.env.OBD_BLE_ALLOW_DUP === '1');
                    }
                    catch (e) {
                        void e;
                    }
                    ...
                    if (secondPhaseExtraMs > 0) {
                        ...
                        scheduleTimeout();
                        if (process.env.OBD_BLE_DEBUG)
                            console.log('[KingbolenEdiag] second-phase timeout extension', { addedMs: secondPhaseExtraMs, newDeadlineInMs: deadline - Date.now() });
                    }
                }, widenMs);
                if (process.env.OBD_BLE_DEBUG)
                    console.log('[KingbolenEdiag] widen timer scheduled', { widenMs });
            };
            ...
            if (abortSignal) {
                ...
                const abortListener = () => {
                    if (found)
                        return; // если уже нашли, игнорируем
            ...
            this.nobleStateChangeHandler = (state) => {
                ...
                if (state === 'poweredOn') {
                    ...
                    try {
                        ...
                        if (process.env.OBD_BLE_DEBUG)
                            console.log('[KingbolenEdiag] startScanning', { services, allowDuplicates: allowDup });
                    }
                    catch (e) {
                        void e;
                    }
            ...
            this.nobleDiscoverHandler = async (peripheral) => {
                ...
                if (process.env.OBD_BLE_DEBUG)
                    console.log('[KingbolenEdiag] discover:', { name, rssi: peripheral.rssi, addr, serviceUuids: svcUuids });
                ...
                if (matchAddr || matchName || matchSvc) {
                    ...
                    if (process.env.OBD_BLE_DEBUG)
                        console.log('[KingbolenEdiag] пропуск peripheral (нет совпадения)');
                }
            };
            ...
            if (noble.state === 'poweredOn') {
                ...
                try {
                    noble.startScanning(services, allowDup);
                    if (process.env.OBD_BLE_DEBUG)
                        console.log('[KingbolenEdiag] startScanning (immediate)', { services, allowDuplicates: allowDup });
                }
                catch (e) {
                    void e;
                }
        ...
        return new Promise((resolve, reject) => {
            (async () => {
                try {
                    ...
                    const trySome = async () => {
                        try {
                            const { characteristics } = await peripheral.discoverSomeServicesAndCharacteristicsAsync([this.options.serviceUUID], [this.options.txCharacteristicUUID, this.options.rxCharacteristicUUID]);
                    ...
                    let characteristics = await trySome();
                    if (characteristics.length === 0) {
                        if (process.env.OBD_BLE_DEBUG)
                            console.log('[KingbolenEdiag] Fallback to discoverAllServicesAndCharacteristics');
                        characteristics = await tryAll();
                    }
                    const norm = (u) => (u || '').toLowerCase().replace(/-/g, '');
                    const expand16 = (u) => (/^[0-9a-f]{4}$/i.test(u) ? `0000${u.toLowerCase()}-0000-1000-8000-00805f9b34fb` : u);
                    ...
                    const toShort = (u) => this.shortUuid(u || '') || '';
                    const wantTxShort = toShort(this.options.txCharacteristicUUID);
                    const wantRxShort = toShort(this.options.rxCharacteristicUUID);
                    const byUuid = (c, wantFull, wantShort) => {
                        const cu = norm(c.uuid);
                        const cShort = toShort(c.uuid);
                        return cu === wantFull || (wantShort && cShort === wantShort);
                    };
                    this.txCharacteristic = characteristics.find((c) => byUuid(c, wantTxFull, wantTxShort));
                    this.rxCharacteristic = characteristics.find((c) => byUuid(c, wantRxFull, wantRxShort));
                    // Фолбэк по свойствам: ищем write*/notify, если точных UUID нет
                    if (!this.txCharacteristic) {
                        this.txCharacteristic = characteristics.find((c) => Array.isArray(c.properties) && (c.properties.includes('write') || c.properties.includes('writeWithoutResponse')));
                    }
                    if (!this.rxCharacteristic) {
                        this.rxCharacteristic = characteristics.find((c) => Array.isArray(c.properties) && (c.properties.includes('notify') || c.properties.includes('indicate')));
                    }
                    if (!this.txCharacteristic || !this.rxCharacteristic) {
                        if (process.env.OBD_BLE_DEBUG)
                            console.log('[KingbolenEdiag] Characteristics not found', {
                                wanted: { tx: this.options.txCharacteristicUUID, rx: this.options.rxCharacteristicUUID },
                                found: characteristics.map((c) => ({ uuid: c.uuid, properties: c.properties }))
                            });
    ...
    async requestDtc() {
        ...
        if (!res.ok || !res.data)
            throw new Error(res.error || 'readDTC failed');
        const mapType = (code) => {
    ...
    async clearDtc() {
        ...
        if (!res.ok)
            throw new Error(res.error || 'clearDTC failed');
        if (!res.data)
            throw new Error('Adapter rejected clear DTC');
    }
