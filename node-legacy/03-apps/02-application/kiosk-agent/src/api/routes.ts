...
import { z } from 'zod';
import type { PaymentModule } from '../payments/module.js';
import type { LockController } from '../locks/index.js';
 ...
 */
export function createPaymentIntentRoute(paymentModule: PaymentModule) {
 ...
 */
export function getPaymentIntentRoute(paymentModule: PaymentModule) {
  return async (req: Request, res: Response): Promise<void> => {
    const { id } = req.params;
 ...
 */
export function cancelPaymentIntentRoute(paymentModule: PaymentModule) {
  return async (req: Request, res: Response): Promise<void> => {
    const { id } = req.params;
 ...
 */
export function confirmPaymentIntentRoute(paymentModule: PaymentModule, environment: string) {
  return async (req: Request, res: Response): Promise<void> => {
    if (environment !== 'DEV') {
      res.status(403).json({
        ok: false,
 ...
 */
export function openLockRoute(lockController: LockController, paymentModule: PaymentModule) {
  return async (req: Request, res: Response): Promise<void> => {
    try {
 ...
 */
export function getLockStatusRoute(lockController: LockController) {
  return async (_req: Request, res: Response): Promise<void> => {
    try {
      ...
      if (thicknessStatus) {
        devices.push({
          deviceType: 'thickness' as const,
          state: thicknessStatus.status === 'unlocked' ? 'opened' as const :
                 thicknessStatus.status === 'locked' ? 'closed' as const :
                 'fault' as const,
      ...
      if (obdStatus) {
        devices.push({
          deviceType: 'obd' as const,
          state: obdStatus.status === 'unlocked' ? 'opened' as const :
                 obdStatus.status === 'locked' ? 'closed' as const :
                 'fault' as const,
 ...
 */
export function emergencyCloseAllLocksRoute(lockController: LockController) {
  return async (_req: Request, res: Response): Promise<void> => {
    try {
