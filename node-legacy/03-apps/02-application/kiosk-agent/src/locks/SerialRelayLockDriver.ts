...
export interface SerialRelayLockDriverConfig {
  port: string;
  baudRate?: number;
  relayChannel?: number;
  openCommand?: Buffer;
...
export class SerialRelayLockDriver implements Lock {
  private readonly config: SerialRelayLockDriverConfig;
  private serialPort?: SerialPort;
  private currentStatus: LockStatus = 'locked';
  private heartbeatTimer?: NodeJS.Timeout;
  private autoCloseTimer?: NodeJS.Timeout;
  private openedAt?: number;
  private deviceName: string;

  constructor(config: SerialRelayLockDriverConfig, deviceName = 'unknown') {
    this.config = {
      baudRate: 9600,
      relayChannel: 1,
   ...
   */
  private buildRelayCommand(channel: number, state: number): Buffer {
