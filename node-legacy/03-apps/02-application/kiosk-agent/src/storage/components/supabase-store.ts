...
export class SupabaseStore {
  ...
  private metrics: {
    operationsTotal: Counter<'operation' | 'status'>;
    operationDuration: Histogram<'operation'>;
    retryAttempts: Counter<'operation'>;
    circuitBreakerStateChanges: Counter<'from_state' | 'to_state'>;
    telemetryLogsDropped: Counter<string>;
    telemetryQueueSize: Gauge<string>;
    circuitBreakerState: Gauge<string>;
  };
  private batchQueue: TelemetryLog[] = [];
  ...
  private forceFlushOnError = false;
  ...
  private recordCircuitBreakerStateChange(fromState: CircuitState, toState: CircuitState): void {
  ...
  async sendTelemetryLog(log: TelemetryLog): Promise<void> {
  ...
  async sendTelemetryBatch(logs: TelemetryLog[]): Promise<void> {
  ...
  async getReportUrl(sessionId: string, format: 'html' | 'pdf'): Promise<string | null> {
...
export function createSupabaseStore(url: string, key: string, registry?: Registry): SupabaseStore {
