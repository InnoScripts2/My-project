...
export class HeartbeatLogger {
  ...
  constructor(logPath: string, intervalMs: number = 30000) {
   ...
   */
  async writeHeartbeat(status: 'ok' | 'degraded' | 'error', message?: string): Promise<void> {
   ...
   */
  async getLastHeartbeat(): Promise<HeartbeatEntry | null> {
    try {
   ...
   */
  async getHeartbeatHistory(limit: number = 100): Promise<HeartbeatEntry[]> {
    try {
      const content = await fs.readFile(this.logPath, 'utf-8');
      const lines = content.trim().split('\n').filter(Boolean);
   ...
   */
  async rotateIfNeeded(maxSizeMb: number = 10): Promise<void> {
    try {
