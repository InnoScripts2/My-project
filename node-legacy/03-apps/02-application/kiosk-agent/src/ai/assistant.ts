import type { ObdDtc, ObdStatus } from "../devices/obd/KingbolenEdiagDriver.js";

export type AiInsightsRequest = {
  vehicle?: { make?: string; model?: string; year?: number };
  dtc?: Array<{ code: string; description?: string }>;
  status?: Partial<ObdStatus>;
};

export type AiInsightsResponse = {
  summary: string; // краткий вывод для клиента
  severity?: { ok?: number; warning?: number; critical?: number };
  recommendations?: Array<{ title: string; steps?: string[] }>;
  disclaimer?: string;
  raw?: string; // оригинальный ответ, если не удалось распарсить JSON
};

function getSupabaseEdgeUrl(): string | undefined {
  const base = process.env.SUPABASE_URL || process.env.NEXT_PUBLIC_SUPABASE_URL;
  if (!base) return undefined;
  try { return new URL("/functions/v1/ai-chat", base).toString(); } catch { return undefined; }
}

export async function fetchAiInsights(payload: AiInsightsRequest, opts?: { timeoutMs?: number }): Promise<AiInsightsResponse> {
  ...
  try {
    ...
    const text: string | undefined = data?.message || data?.choices?.[0]?.message?.content;
    if (!text) return { summary: "Нет ответа от ИИ", disclaimer: "Сервис ИИ недоступен.", raw: JSON.stringify(data).slice(0, 1000) };
...
export function buildPayloadFromDtc(dtc: ObdDtc[], status?: Partial<ObdStatus>): AiInsightsRequest {
  return {
