...
import type { LogEntry, LogChannel } from './CentralizedLogger.js';
...
export interface AnomalyPattern {
  ...
  detector: (entries: LogEntry[], timeWindowMs: number) => AnomalyMatch | null;
}
 ...
 */
export const BUILT_IN_PATTERNS: AnomalyPattern[] = [
  {
    id: 'error_storm',
    name: 'Шторм ошибок',
    ...
    detector: (entries, timeWindowMs) => {
      ...
      const recentErrors = entries.filter(
        (e) => (e.level === 'error' || e.level === 'fatal') && e.timestamp >= cutoff
      );
  ...
  },
  {
    ...
    detector: (entries, timeWindowMs) => {
      ...
      const connectionFailures = entries.filter(
        (e) =>
          e.channel === 'obd' &&
  ...
  },
  {
    ...
    detector: (entries, timeWindowMs) => {
      ...
      const paymentWarnings = entries.filter(
        (e) =>
          e.channel === 'payments' &&
          e.level === 'warn' &&
          e.timestamp >= cutoff &&
  ...
  },
  {
    ...
    detector: (entries, timeWindowMs) => {
      ...
      const errorCounts = new Map<string, LogEntry[]>();
  ...
  },
  {
    ...
    detector: (entries, timeWindowMs) => {
      ...
      // Считаем по каналам
      const channelCounts = new Map<LogChannel, number>();
...
export class AnomalyDetector {
  private readonly patterns: AnomalyPattern[];
  private readonly detectedAnomalies: AnomalyMatch[] = [];
  private readonly maxAnomalies: number;

  constructor(options?: { patterns?: AnomalyPattern[]; maxAnomalies?: number }) {
   ...
   */
  detect(entries: LogEntry[], timeWindowMs: number = 10 * 60_000): AnomalyMatch[] {
    const detected: AnomalyMatch[] = [];
   ...
   */
  getDetectedAnomalies(options?: { since?: string; severity?: AnomalyMatch['severity'] }): AnomalyMatch[] {
  ...
  private recordAnomaly(anomaly: AnomalyMatch): void {
