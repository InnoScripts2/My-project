...
test('POST /reports/generate writes HTML to outbox', { timeout: 15000 }, async (t) => {
  ...
  t.after(() => {
    ...
    if (prevOutbox === undefined) delete process.env.REPORTS_OUTBOX; else process.env.REPORTS_OUTBOX = prevOutbox;
...
test('POST /reports/send returns 501 when SMTP is not configured (or 200 in DEV with simulation)', { timeout: 15000 }, async (t) => {
  ...
  t.after(() => {
    server.close();
    // restore SMTP env
    if (smtpEnv.host !== undefined) process.env.SMTP_HOST = smtpEnv.host; else delete process.env.SMTP_HOST;
    if (smtpEnv.port !== undefined) process.env.SMTP_PORT = smtpEnv.port; else delete process.env.SMTP_PORT;
    if (smtpEnv.secure !== undefined) process.env.SMTP_SECURE = smtpEnv.secure; else delete process.env.SMTP_SECURE;
    if (smtpEnv.user !== undefined) process.env.SMTP_USER = smtpEnv.user; else delete process.env.SMTP_USER;
    if (smtpEnv.pass !== undefined) process.env.SMTP_PASS = smtpEnv.pass; else delete process.env.SMTP_PASS;
    if (smtpEnv.from !== undefined) process.env.SMTP_FROM = smtpEnv.from; else delete process.env.SMTP_FROM;
  });
  const baseUrl = await waitForServer(server);
...
test('POST /reports/send-sms returns 501 when SMS not configured (or 200 in DEV with provider=dev)', { timeout: 15000 }, async (t) => {
  ...
  t.after(() => {
    server.close();
    if (prevProvider !== undefined) process.env.SMS_PROVIDER = prevProvider; else delete process.env.SMS_PROVIDER;
  });
  const baseUrl = await waitForServer(server);
  const payload = {
    data: {
