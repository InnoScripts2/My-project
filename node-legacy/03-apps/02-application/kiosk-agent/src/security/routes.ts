...
import type { SecurityMetrics } from './metrics.js';
import type { AuditCategory } from './types.js';

export function createSecurityRoutes(metrics?: SecurityMetrics): Router {
  const router = Router();
    ...
    try {
      ...
      if (scheduledTime) {
        ...
        if (downloadResult.signaturePath) {
          ...
          const signatureValid = await updateManager.verifySignature(
            downloadResult.artifactPath,
            downloadResult.signaturePath,
            publicKey
          );
