      ...
      <a-spin :spinning="configStore.loading">
        ...
        >
          <a-collapse v-model:activeKey="activeKey" :bordered="false">
            <a-collapse-panel key="devices" header="Devices Configuration">
              <a-form-item label="OBD Port" name="obdPort">
                <a-input v-model:value="configForm.obdPort" placeholder="e.g., COM3" />
              </a-form-item>
              <a-form-item label="OBD Baud Rate" name="obdBaudRate">
                <a-input-number v-model:value="configForm.obdBaudRate" :min="9600" :max="115200" style="width: 100%;" />
              </a-form-item>
              <a-form-item label="Thickness Device ID (BLE)" name="thicknessDeviceId">
                <a-input v-model:value="configForm.thicknessDeviceId" placeholder="e.g., AA:BB:CC:DD:EE:FF" />
              </a-form-item>
            </a-collapse-panel>

            <a-collapse-panel key="payments" header="Payments Configuration">
              <a-form-item label="YooKassa Shop ID" name="yookassaShopId">
                <a-input v-model:value="configForm.yookassaShopId" placeholder="Shop ID" />
              </a-form-item>
              <a-form-item label="YooKassa API Key" name="yookassaApiKey">
                <a-input-password v-model:value="configForm.yookassaApiKey" placeholder="API Key (optional)" />
              </a-form-item>
            </a-collapse-panel>

            <a-collapse-panel key="reports" header="Reports Configuration">
              <a-form-item label="SMTP Host" name="smtpHost">
                <a-input v-model:value="configForm.smtpHost" placeholder="e.g., smtp.example.com" />
              </a-form-item>
              <a-form-item label="SMTP Port" name="smtpPort">
                <a-input-number v-model:value="configForm.smtpPort" :min="1" :max="65535" style="width: 100%;" />
              </a-form-item>
              <a-form-item label="SMTP User" name="smtpUser">
                <a-input v-model:value="configForm.smtpUser" placeholder="SMTP username (optional)" />
              </a-form-item>
              <a-form-item label="SMTP Password" name="smtpPassword">
                <a-input-password v-model:value="configForm.smtpPassword" placeholder="SMTP password (optional)" />
              </a-form-item>
            </a-collapse-panel>
          </a-collapse>
          ...
          <a-form-item style="margin-top: 24px;">
            <a-space>
              <a-button type="primary" html-type="submit" :loading="configStore.loading">
                Save Changes
              </a-button>
...
async function onSaveConfig() {
  ...
  if (result.success) {
    ...
    notification.error({ message: 'Failed to save configuration', description: result.error });
  }
}
