...
import type { ObdDriver, ObdMode, PidIdentifier, DtcCode } from './ObdDriverInterface.js';

export interface UsbObdConfig {
  usbPath?: string;
  autoDetect?: boolean;
  baudRate?: number;
...
export class UsbObdDriver extends EventEmitter implements ObdDriver {
  private serialPort: SerialPort | null = null;
  private connected = false;
  private config: Required<UsbObdConfig>;
  private deviceInfo: UsbDeviceInfo = {};
  private responseBuffer = '';

  constructor(config: UsbObdConfig) {
    super();
    this.config = {
   ...
   */
  async detectUsbAdapter(): Promise<string> {
    ...
    const ftdiPort = obdPorts.find(port => 
      port.vendorId?.toUpperCase().includes('0403')
    );
   ...
   */
  getUsbDeviceInfo(): UsbDeviceInfo {
   ...
   */
  private parseDtcCode(hex: string): string {
