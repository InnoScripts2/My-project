...
function getOrchestrator() {
    if (!orchestratorInstance) {
        ...
        const driver = isDev ? new FakeObdDevice({ scenario: 'DtcPresent' }) : obdConnectionManager;
...
export function createObdRoutes() {
    const router = Router();
    router.get('/api/obd/status', async (_req, res) => {
        try {
    ...
    router.post('/api/obd/connect', async (req, res) => {
    ...
    router.post('/api/obd/disconnect', async (_req, res) => {
        try {
    ...
    router.get('/api/obd/dtc', async (_req, res) => {
        try {
            const snapshot = obdConnectionManager.getSnapshot();
    ...
    router.post('/api/obd/dtc/clear', async (req, res) => {
    ...
    router.get('/api/obd/pids/live', async (_req, res) => {
        try {
            const snapshot = obdConnectionManager.getSnapshot();
            if (snapshot.state !== 'connected') {
                res.status(503).json({
    ...
    router.post('/api/obd/orchestrator/connect', async (_req, res) => {
        try {
    ...
    router.post('/api/obd/orchestrator/scan', async (req, res) => {
    ...
    router.get('/api/obd/orchestrator/status', async (_req, res) => {
        try {
            const orchestrator = getOrchestrator();
    ...
    router.get('/api/obd/orchestrator/results/:sessionId', async (req, res) => {
        try {
    ...
    router.post('/api/obd/orchestrator/clear-dtc', async (req, res) => {
        const parsed = clearDtcSchema.safeParse(req.body);
        if (!parsed.success) {
            res.status(400).json({
                error: 'invalid_payload',
    ...
    router.post('/api/obd/orchestrator/disconnect', async (_req, res) => {
        try {
            const orchestrator = getOrchestrator();
