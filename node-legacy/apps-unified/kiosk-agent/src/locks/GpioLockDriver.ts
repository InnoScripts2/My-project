...
export interface GpioLockDriverConfig {
  pin: number;
  activeHigh?: boolean;
 ...
 */
export class GpioLockDriver implements Lock {
  private readonly config: GpioLockDriverConfig;
  private currentStatus: LockStatus = 'locked';
  private autoCloseTimer?: NodeJS.Timeout;
  private openedAt?: number;
  private deviceName: string;

  constructor(config: GpioLockDriverConfig, deviceName = 'unknown') {
    this.config = {
      activeHigh: true,
   ...
   */
  private async simulateGpioWrite(_value: number): Promise<void> {
