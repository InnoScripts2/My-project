...
import type { DiagnosticTimelineEvent, DiagnosticOperation, DiagnosticState } from '../../devices/obd/DiagnosticSessionManager.js'
import type {
...
function summarizeFromEvents(events: DiagnosticTimelineEvent[], options: DiagnosticsEventStoreSummarizeOptions = {}): DiagnosticsEventStoreSummary {
  ...
  const dateKey = (iso?: string): string | null => {
    if (!iso) return null
  ...
  for (const e of filtered) {
    ...
    if (d) {
      ...
      if (!bucket) {
        bucket = { total: 0, success: 0, failure: 0, byOp: OPERATIONS.reduce((acc, op) => { acc[op] = { success: 0, failure: 0 }; return acc }, {} as any) }
  ...
  function classifyTrend(current: any, baseline: any): DiagnosticsEventStoreTrendEntry {
    ...
    if (successRateDelta >= 0.05 || failureRateDelta <= -0.05) return { totalDelta, successRateDelta, failureRateDelta, status: 'improving', reason: 'Растёт успех/падает отказ', confidence: 0.7 }
    if (successRateDelta <= -0.05 || failureRateDelta >= 0.05 || totalDelta <= -1) return { totalDelta, successRateDelta, failureRateDelta, status: 'regressing', reason: 'Хуже успех/больше отказов/меньше объём', confidence: 0.7 }
  ...
  let currentSinceMs = parseTs((filtered[0] as any)?.at) ?? captureMs
  const pushSeg = (state: DiagnosticState, from?: number, to?: number) => {
...
export function createSupabaseDiagnosticsEventStore(): DiagnosticsEventStore {
  ...
  const store: DiagnosticsEventStore = {
    ...
    getRecent(): DiagnosticTimelineEvent[] { return ring.slice(Math.max(0, ring.length - 100)) },
