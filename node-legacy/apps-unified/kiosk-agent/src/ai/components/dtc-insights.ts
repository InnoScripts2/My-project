...
const DTC_DATABASE: Record<string, DtcDefinition> = {
  P0300: {
...
export function getDtcInsight(dtcCode: string): DtcInsight {
  ...
  const prefix = normalizedCode[0] as DtcPrefix;
  const fallback = generateFallbackInsight(normalizedCode, prefix);

  return fallback;
}

function generateFallbackInsight(code: string, prefix: DtcPrefix): DtcInsight {
  const prefixMap: Record<DtcPrefix, { category: string; severity: DtcSeverity }> = {
  ...
  const info = prefixMap[prefix] || { category: 'Неизвестная категория', severity: 'MEDIUM' as DtcSeverity };
