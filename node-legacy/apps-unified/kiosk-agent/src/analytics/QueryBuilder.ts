...
export class QueryBuilder {
   ...
   */
  buildSessionsQuery(filter: SessionsFilter): string {
   ...
   */
  buildRevenueQuery(filter: RevenueFilter): string {
    const groupBy = filter.groupBy || 'day';
    const dateFunc = this.getDateTruncFunction(groupBy);
   ...
   */
  buildErrorsQuery(filter: ErrorsFilter): string {
    return `
      SELECT
        dtc_code AS code,
        description,
   ...
   */
  buildTrendsQuery(filter: TrendsFilter): string {
    const groupBy = filter.groupBy || 'day';
    const dateFunc = this.getDateTruncFunction(groupBy);
   ...
   */
  private getDateTruncFunction(groupBy: string): string {
    switch (groupBy) {
      case 'day':
