...
describe('SMS Adapter', () => {
  describe('getSmsConfigFromEnv', () => {
    it('возвращает null если нет обязательных переменных', () => {
      ...
      if (oldKey) process.env.SMS_API_KEY = oldKey
      if (oldSender) process.env.SMS_SENDER_ID = oldSender
    })
    ...
    it('возвращает конфигурацию с обязательными полями', () => {
      ...
      if (oldSender) process.env.SMS_SENDER_ID = oldSender
      else delete process.env.SMS_SENDER_ID
    })
    ...
    it('парсит опциональный API URL', () => {
      ...
      if (oldUrl) process.env.SMS_API_URL = oldUrl
      else delete process.env.SMS_API_URL
    })
  })
  ...
  describe('DevSmsAdapter', () => {
    it('логирует payload в DEV режиме', async () => {
      ...
      if (oldEnv) process.env.AGENT_ENV = oldEnv
      else delete process.env.AGENT_ENV
    })
