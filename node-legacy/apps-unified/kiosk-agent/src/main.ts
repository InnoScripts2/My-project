...
import { createDtcRoutes } from './api/routes/dtc.routes.js';
...
export class KioskAgent {
  ...
  app.use(helmet());
    ...
    app.use((req: Request, res: Response, next) => {
      ...
      const acrh = (req.headers['access-control-request-headers'] as string) || 'Content-Type, Authorization, Accept, X-Requested-With';
    ...
    app.get('/api/auth/ping', (_req: Request, res: Response) => {
  ...
  app.use('/api/obd', createObdHealthRoutes(obdConnectionManager as any, lockController));
  ...
  const obdPaymentAdapter = new ObdPaymentAdapter(paymentsModule as any);
  ...
  app.use(createDtcRoutes());
    ...
    try {
      ...
      const mod: any = await import('socket.io');
      const SocketIOServer = mod.Server as any;
      ...
      setInterval(() => {
        try { this.io?.emit('ping', { ts: new Date().toISOString() }); }
        catch (e) { console.debug('[io] emit error', (e as Error)?.message); }
      }, 20000);
...
const env = (process.env.AGENT_ENV as 'DEV' | 'QA' | 'PROD') || 'DEV';
