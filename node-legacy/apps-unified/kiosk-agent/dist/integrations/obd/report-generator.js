...
export class ObdReportGenerator {
    ...
    async saveReport(sessionId, pdfBuffer) {
    ...
    generateHtmlReport(sessionData, options) {
    ...
    getStandardTemplate(sessionData, recommendations) {
        const dtcRows = sessionData.dtcCodes.map((dtc) => {
      ...
      `;
        ...
        const pidsRows = sessionData.pidsSnapshot.map((pid) => {
...
<body>
  ...
  <footer>
    ...
    getHybridTemplate(sessionData, recommendations) {
    ...
    getOverallStatusClass(sessionData) {
        const dtcCodes = sessionData.dtcCodes || [];
        const hasCritical = dtcCodes.some((dtc) => dtc.severity === 'critical');
        const hasWarning = dtcCodes.some((dtc) => dtc.severity === 'warning');
        if (hasCritical)
            return 'critical';
        if (hasWarning || dtcCodes.length > 0)
            return 'warning';
        return 'ok';
    }
    getOverallStatusText(sessionData) {
        const dtcCodes = sessionData.dtcCodes || [];
        const hasCritical = dtcCodes.some((dtc) => dtc.severity === 'critical');
        const hasWarning = dtcCodes.some((dtc) => dtc.severity === 'warning');
        if (hasCritical)
...
// ESM: вычисляем директорию текущего файла
const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);
export const obdReportGenerator = new ObdReportGenerator(process.env.TEMPLATES_PATH || path.join(__dirname, '../../templates/obd'));
