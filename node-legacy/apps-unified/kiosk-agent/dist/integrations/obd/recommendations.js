export class RecommendationsEngine {
    static generateRecommendations(sessionData) {
        const recommendations = [];
        const dtcCodes = sessionData.dtcCodes || [];
        const pidsSnapshot = sessionData.pidsSnapshot || [];
        // Analyze DTC codes by type
        const powertrainDtc = dtcCodes.filter((dtc) => dtc.code?.startsWith('P'));
        const chassisDtc = dtcCodes.filter((dtc) => dtc.code?.startsWith('C'));
        const bodyDtc = dtcCodes.filter((dtc) => dtc.code?.startsWith('B'));
        const networkDtc = dtcCodes.filter((dtc) => dtc.code?.startsWith('U'));
        // Powertrain issues
        if (powertrainDtc.length > 0) {
            const critical = powertrainDtc.filter((dtc) => dtc.severity === 'critical').length;
            if (critical > 0) {
    ...
    static findPidValue(pidsSnapshot, pidName) {
        const pid = pidsSnapshot.find((p) => p.name === pidName || p.id === pidName);
