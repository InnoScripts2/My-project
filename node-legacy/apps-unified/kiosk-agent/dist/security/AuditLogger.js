...
export class AuditLogger {
    constructor(logsDir, retentionDays) {
    ...
    async logEvent(category, action, userId, details, sourceIp, result = 'success', errorMessage) {
        const entry = {
            eventId: uuidv4(),
    ...
    async queryLogs(filter) {
        ...
        try {
            ...
            console.error(`Failed to query logs: ${error instanceof Error ? error.message : String(error)}`);
            return results;
        }
    }
            ...
            for (const file of logFiles) {
                ...
                if (fileDate < cutoffDate) {
                    ...
                    try {
                        ...
                        console.error(`Failed to archive ${file}: ${error instanceof Error ? error.message : String(error)}`);
                    }
                }
            }
            return deletedCount;
    ...
    async readLogFile(filePath, filter) {
        ...
        try {
            ...
            console.error(`Failed to read log file ${filePath}: ${error instanceof Error ? error.message : String(error)}`);
        }
        return entries;
    }
}
