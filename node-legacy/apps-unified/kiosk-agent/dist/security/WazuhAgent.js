...
export class WazuhAgent {
    ...
    async configureAgent(config) {
        ...
        try {
            ...
            throw new Error(`Failed to configure agent: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    async startAgent() {
        try {
    ...
    async getStatus() {
        try {
            ...
            const version = this.getInstalledVersion();
            const policies = [];
            if (output.includes('syscheck'))
                policies.push('FIM');
            if (output.includes('rootcheck'))
                policies.push('RootkitDetection');
            if (output.includes('vulnerability'))
                policies.push('VulnerabilityScanning');
            return {
                installed: true,
                version,
                connected,
    ...
    generateOssecConfig(config) {
        ...
        const fimDirectories = fimPolicy?.settings?.directories;
        const fimRealTime = fimPolicy?.settings?.realTime;
        const rootkitInterval = rootkitPolicy?.settings?.interval;
        const vulnScanTime = vulnPolicy?.settings?.scanTime;
