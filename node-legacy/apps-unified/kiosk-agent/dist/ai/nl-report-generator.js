...
export class NaturalLanguageReportGenerator {
    ...
    async generateClientFriendlyReport(diagnostics) {
    ...
    async explainSeverity(dtcCode, severity) {
        const explanations = {
    ...
    async generateActionRecommendations(dtcCodes) {
    ...
    async estimateRepairCost(dtcCodes) {
    ...
    fallbackReport(diagnostics) {
        const issues = diagnostics.dtcCodes.map(code => {
    ...
    getClientFriendlyExplanation(code) {
        const explanations = {
    ...
    calculateOverallHealth(critical, high, medium) {
        if (critical > 0)
            return 'poor';
        if (high > 1)
            return 'poor';
    ...
    generateSummary(critical, high, medium, total) {
        if (total === 0)
            return 'Проблем не обнаружено. Все системы в норме.';
        const parts = [`Обнаружено ${total} ${this.pluralize(total, 'проблема', 'проблемы', 'проблем')}`];
        if (critical > 0) {
    ...
    generateNextSteps(critical, high) {
        if (critical > 0) {
