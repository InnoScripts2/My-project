...
function resolveMaxDays(value) {
    if (typeof value !== 'number' || !Number.isFinite(value) || value <= 0) {
        return DEFAULT_MAX_DAILY_BUCKETS;
    }
    return Math.floor(value);
}
function buildDailySummaryEntries(buckets, maxDays) {
...
function createDailyOperationAccumulator() {
...
function createRollingOperationBuckets() {
    return OPERATIONS.reduce((acc, operation) => {
        acc[operation] = {
            total: 0,
            success: 0,
...
function createEmptyRollingEntry(windowDays, captureMs) {
...
function applyOperationMetrics(counts, durations) {
...
function computeMeanTimeToRecovery(events) {
    ...
    for (let index = 0; index < events.length; index += 1) {
        ...
        if (!(event.type === 'operation' && event.outcome === 'failure'))
            continue;
...
function buildConnectionSummaryFromEvents(events, captureMs, since) {
    ...
    const pushSegment = (state, fromMs, toMs) => {
...
function computeRollingWindow(operationEvents, captureMs, windowDays) {
...
function classifyTrendStatus(currentTotal, baselineTotal, successRateDelta, failureRateDelta, totalDelta) {
    const totalEvents = currentTotal + baselineTotal;
    const eventSupport = clamp(totalEvents / (TREND_MIN_EVENTS_FOR_STATUS * 2));
    const deltaSupportBase = Math.max(Math.abs(successRateDelta) / TREND_SUCCESS_RATE_THRESHOLD, Math.abs(failureRateDelta) / TREND_FAILURE_RATE_THRESHOLD, Math.abs(totalDelta) / TREND_TOTAL_DELTA_THRESHOLD);
...
function computeTrendEntry(current, baseline) {
    ...
    const { status, reason, confidence } = classifyTrendStatus(current.total, baseline.total, successRateDelta, failureRateDelta, totalDelta);
    return {
        totalDelta,
        successRateDelta,
        failureRateDelta,
...
function computeOperationTrends(current, baseline) {
    const result = {};
    for (const operation of OPERATIONS) {
        ...
        const totalDelta = currentAvg - baselineAvg;
        const { status, reason, confidence } = classifyTrendStatus(currentMetrics.total, baselineMetrics.total, successRateDelta, failureRateDelta, totalDelta);
        result[operation] = {
            totalDelta,
            successRateDelta,
            failureRateDelta,
...
export function createDiagnosticsEventStore(options = {}) {
  ...
  `);
    ...
    return {
        ...
        summarize(options = {}) {
            ...
            try {
                const rows = selectWindowStmt.all({ since: since ?? null });
