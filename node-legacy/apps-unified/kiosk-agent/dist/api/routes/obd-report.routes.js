...
export function createObdReportRoutes() {
    const router = Router();
    // Generate report
    router.post('/api/obd/report/generate', async (req, res) => {
    ...
    // Send report
    router.post('/api/obd/report/send', async (req, res) => {
        ...
        try {
            ...
            if (email) {
                ...
                if (mailConfig) {
                    const outboxRoot = process.env.REPORTS_OUTBOX || '/tmp/outbox';
                    const reportPath = `${outboxRoot}/obd/${sessionId}.html`;
                    try {
                        await sendReportEmail(email, 'Отчёт OBD-II диагностики', reportPath, mailConfig);
                        sentTo.push(email);
                    }
                    catch (err) {
    ...
    // Get report
    router.get('/api/obd/report/:sessionId', async (req, res) => {
