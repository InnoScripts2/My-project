...
export function createReportsRoutes() {
    ...
    router.get('/api/reports/:reportId/preview', async (req, res) => {
    ...
    router.get('/api/reports/:reportId/download', async (req, res) => {
        const { reportId } = req.params;
        if (!reportId) {
            res.status(400).send('Report ID required');
            return;
    ...
    router.post('/api/reports/:reportId/send', async (req, res) => {
        const { reportId } = req.params;
