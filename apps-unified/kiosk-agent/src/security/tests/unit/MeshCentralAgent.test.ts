...
const tempRoot = path.join(os.tmpdir(), 'kiosk-agent-tests', 'meshcentral');
...
describe('MeshCentralAgent', () => {
  ...
  it('should handle file upload with error handling', async () => {
    const agent = new MeshCentralAgent();
    const result = await agent.uploadFile('/nonexistent/file.txt', path.join(tempRoot, 'upload.txt'));
  ...
  it('should handle file download with error handling', async () => {
    const agent = new MeshCentralAgent();
    const result = await agent.downloadFile('/nonexistent/remote.txt', path.join(tempRoot, 'download.txt'));
