...
export class UpdateManager {
  ...
  private async createTempDir(prefix: string): Promise<string> {
    const baseDir = path.join(os.tmpdir(), 'kiosk-agent');
  ...
  const tempDir = await this.createTempDir('update');
  ...
  async verifySignature(
    ...
    const tempDir = await this.createTempDir('verify');
  ...
  async applyUpdate(artifactPath: string): Promise<ApplyResult> {
    try {
      ...
      const tempExtractDir = await this.createTempDir('extract');
