...
import type { KingbolenEdiagDriver } from '@selfservice/obd-diagnostics';

class FakeObdDriver implements Pick<KingbolenEdiagDriver, 'disconnect' | 'getMetrics'> {
  private readonly metrics = { failedCommands: 0 };

  async disconnect(): Promise<void> {
    // no-op for tests
  }

  getMetrics(): { failedCommands: number } {
    return { ...this.metrics };
  }
}

function createDriver(): KingbolenEdiagDriver {
  return new FakeObdDriver() as unknown as KingbolenEdiagDriver;
}

describe('ObdConnectionPool', () => {
  it('should acquire and release connections', async () => {
  const pool = new ObdConnectionPool(3, 30000, 3, createDriver);
  ...
  const pool = new ObdConnectionPool(2, 30000, 3, createDriver);
  ...
  const pool = new ObdConnectionPool(1, 30000, 3, createDriver);
