        ...
        <template #bodyCell="{ column, record }">
          ...
          <template v-else-if="column.key === 'actions'">
            <a-space>
              <a-button
                size="small"
                @click="onViewExecutions(record.workflowId)"
              >
                Executions
              </a-button>
              <a-button
                size="small"
                @click="onTriggerWorkflow(record.workflowId)"
              >
                Trigger
              </a-button>
...
async function onSubmitCreate() {
  try {
    await workflowsStore.createWorkflow({
      ...
      trigger: {
        type: "webhook",
        config: { webhookPath: `/webhooks/${formData.value.name}` },
      },
      steps: [],
    });
...
function onEditWorkflow(_workflow: any) {
  message.info(
    "Edit workflow functionality - use REST API for advanced configuration"
  );
}
